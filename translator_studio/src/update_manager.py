...
def _request_bytes(url: str, timeout: int = 20) -> bytes:
    request = urllib.request.Request(
        url,
        headers={
            "User-Agent": "SubDub-Forge-Updater",
...
def _safe_relative(value: str) -> Path:
    ...
    if relative.name in PROTECTED_FILES or relative.parts[0] in PROTECTED_DIRS:
        raise UpdateError(f"更新清单试图修改受保护内容：{value}")
    return relative
...
def _zip_entries(archive: zipfile.ZipFile) -> tuple[str, dict[str, zipfile.ZipInfo]]:
    files = [info for info in archive.infolist() if not info.is_dir()]
    if not files or "/" not in files[0].filename:
        raise UpdateError("GitHub 更新包目录结构无效。")
    prefix = files[0].filename.split("/", 1)[0] + "/"
    entries = {info.filename[len(prefix):]: info for info in files if info.filename.startswith(prefix)}
...
def download_and_apply(remote: dict[str, Any], progress: Callable[[str], None] | None = None) -> Path:
    ...
    with archive:
        ...
        old_manifest = _load_manifest_bytes(MANIFEST_FILE.read_bytes()) if MANIFEST_FILE.exists() else {"files": []}
        old_paths = {_safe_relative(str(value)) for value in old_manifest.get("files", [])}
        affected = old_paths | new_paths
        stamp = time.strftime("%Y%m%d_%H%M%S")
        backup = ROOT / "output" / "_internal" / "update_backups" / f"v{local['version']}_{stamp}"
        backup.mkdir(parents=True, exist_ok=True)
        existed_before: set[Path] = set()

        report(f"正在备份当前 v{local['version']} 程序文件……")
        ...
        try:
            ...
            for relative in sorted(new_paths, key=lambda path: path.as_posix()):
                ...
                temp.write_bytes(archive.read(info))
    ...
    report(f"已安装 v{new_version}，备份位置：{backup}")
    return backup
