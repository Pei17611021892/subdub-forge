Option Explicit
' Silent launcher for Translator Studio.
...
appScript = root & "\translator_studio\src\gui_launcher.py"
...
If Not fso.FileExists(pythonExe) Or Not fso.FileExists(pythonwExe) Then
    shell.Popup "Project virtual environment was not found." & vbCrLf & _
        "Run: python -m venv venv", 0, "SubDub Forge", 16
    WScript.Quit 1
End If

shell.CurrentDirectory = root
checkCode = shell.Run(Quote(pythonExe) & " -c " & Quote("import tkinter, yaml"), 0, True)
If checkCode <> 0 Then
    shell.Popup "Application dependencies are missing." & vbCrLf & _
